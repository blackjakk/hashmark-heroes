...
- **Back / home is NOT unified** — some screens have a back button (and
  shouldn't, e.g. mid-draft), others lack one (and should), some
...
| 6 | playoffs | **app shell** `_frnRenderActiveTab` → `renderFrnPlayoffs` (offseason ~4958) | play/sim your game; manage roster via the tabs | hero "▶ PLAY GAME" / ⏩ Sim → `frnPlayGame` / `frnSimPlayoffGame`; "▶ ADVANCE ROUND" → `frnConfirmAdvancePlayoffRound` | **command-center Overview** (`_renderPlayoffHero` matchup + win%, bracket "season map", `_renderPlayoffPrepTasks`), 8-team bracket, All-Pro Bowl |
...
| Playoffs | none — **runs in the app shell** | ✅ **FIXED**: playoffs render inside the tabbed dashboard (Overview = hero + bracket); the tab bar is the nav. Removed the standalone BSPN sub-nav + "Return to Main Screen". |
...
is ad-hoc. ~~milestone screens aren't real phases~~ — ✅ **FIXED:** the season
recap and post-draft grade are now real `franchise.phase` values
(`season_recap` / `draft_grade`) the router drives, not boolean overlays. (The
recap was in fact orphaned dead code before — `_bumpWeek` flips to
`playoffs_pending` at season end, so the old `phase==="regular" && week>WEEKS`
trigger could never fire. It now renders again as a real phase.)
...
2. **Back semantics by screen class:**
   ...
   - *Locked transactional screens* (FA, FA cuts, mid-draft):
     replace the dangerous back with **"Home (progress saved)"** that returns to
     the dashboard without losing draft state — or disable + tooltip.
   - *Dashboard tabs* (regular season **+ playoffs**): Back is a no-op; the tab
     bar IS the nav. ✅ **Playoffs solved this way** — they now run inside the
     dashboard, so roster management stays available and there's no mid-bracket trap.
3. **Replace `window.confirm()`** with one styled modal component
   (title/body/confirm/cancel) used by all `frnConfirm*`.
4. ✅ **DONE — milestones promoted to real phases** (`season_recap`,
   `draft_grade`). The router drives them; the nav bar reads `_FRN_PHASE_NAV`;
   both are refresh-safe (`draft_grade` rebuilds the class via
   `_myDraftPicksForGrade()` from `draftLog`); auto-sim shortcuts bypass the
   recap; a save-migration maps legacy mid-recap / mid-grade saves. Verified
   headless (`_phase_verify.js`, 15/15).
...
**Also done (2026-06 session — visual overhaul + playoffs in the dashboard):**
- 🎨 **HH modern visual overhaul** — dashboard-wide dark theme keyed to each
  team's color with luminance-based **auto-contrast** (legible for all 32 teams,
  light and near-black), Bricolage Grotesque type, rounded elevated cards, and
  unified tables / buttons / inputs / pill tabs & sub-navs / confirm modals.
  All scoped under `#franchiseHome`. The League "BSPN" screens, the in-game
  **live broadcast**, the **draft** screens, and the confirm modals were dark-
  skinned too (the `--bl*` token palette was flipped light→dark).
- 🏟 **Playoffs moved into the tabbed dashboard** — `showFranchiseDashboard`
  now renders the app shell during `phase==="playoffs"` (not just `regular`),
  so Depth Chart / Front Office / League stay reachable in the postseason.
  `_frnRenderAppShell` self-shows; `renderFrnPlayoffs` ensures the shell on its
  many direct render paths and **hides the milestone nav bar** in playoffs
  (the tabs are the nav). Identity sub-line shows "PLAYOFFS".
- 🏆 **Playoff Overview = command center** — `renderFrnPlayoffs` now mirrors the
  regular Overview: `_renderPlayoffHero` (next playoff game, win%, elimination
  stakes, PLAY GAME — same `.frn-hero-card`) + `_renderPlayoffPrepTasks` (slim
  depth/snaps/injuries checklist) on top, with the bracket demoted to a "season
  map" `.frn-card-box` and the road-to-championship / around-the-round rails
  below. Embedded bracket modules use a contained `.bspnlive-root.embed`.
- ✏️ **"Sim to Playoffs"** copy fixed — it lands on the **season recap** (where
  you start the playoffs), not the raw bracket.
